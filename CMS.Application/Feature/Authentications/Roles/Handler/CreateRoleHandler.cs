...
{
    public class CreateRoleHandler(IRoleServices roleServices) : IRequestHandler<CreateRole, IResult<RoleDto>>
    {
        public async Task<IResult<RoleDto>> Handle(CreateRole request, CancellationToken cancellationToken)
        {
            return await roleServices.Create(request.Role, cancellationToken);
        }
    }
}
